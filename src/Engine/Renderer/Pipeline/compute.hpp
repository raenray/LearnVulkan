...
public:
    ComputePipeline(VkDevice device, const ComputePipelineDesc& desc,
...
inline ComputePipeline::ComputePipeline(VkDevice device, const ComputePipelineDesc& desc,
...
inline std::vector<char> ComputePipeline::readFile(const std::string& path) {
    ...
    if (!file.is_open()) throw std::runtime_error("ComputePipeline: failed to open '" + path + "'");
