...
PipelineCache::~PipelineCache() { clear(); if (cache_) vkDestroyPipelineCache(device_, cache_, nullptr); }
...
void PipelineCache::remove(const std::string& name) {
    ...
    if (it != pipelines_.end()) {
        if (it->second) vkDestroyPipeline(device_, it->second, nullptr);
        pipelines_.erase(it);
    }
}
...
std::vector<uint8_t> PipelineCache::getData() const {
    ...
    if (size > 0) vkGetPipelineCacheData(device_, cache_, &size, data.data());
    return data;
}
